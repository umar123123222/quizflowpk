...
const ThemeToggle = ({ variant = "dashboard" }: ThemeToggleProps) => {
  ...
  return (
    <button
      onClick={toggle}
      className="inline-flex items-center gap-1.5 rounded-full border border-[hsl(var(--dashboard-border))] bg-[hsl(var(--dashboard-card))] px-3 py-1 font-mono text-[10px] tracking-[0.12em] uppercase text-white/50 transition-colors hover:text-[hsl(var(--dashboard-gold))] hover:border-[hsl(var(--dashboard-gold))/0.3]"
    >
      {isDark ? <Sun className="h-3 w-3" /> : <Moon className="h-3 w-3" />}
      {isDark ? "Light mode" : "Dark mode"}
    </button>
